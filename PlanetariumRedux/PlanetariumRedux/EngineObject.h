...
struct Transform
{
	glm::vec3 position = glm::vec3(0.f, -1.8f, -1.f);
	glm::vec3 rotation = glm::vec3(0.f, 0.f, 0.f);
	glm::vec3 scale = glm::vec3(.05f, .05f, .05f);

	Transform() 
	{
		position = glm::vec3(0.f, -1.8f, -1.f);
...
public:
	...
	EngineObject()
 	{
		...
		terrain = PerlinClass(4, 0.8);
		tBonus = PerlinClass(5, 0.9);
		Transform();
	}
	...
	virtual void transform(glm::mat4* model)
	{
		(*model) = glm::translate((*model), m_transform.position);
		(*model) = glm::scale((*model), m_transform.scale);
		(*model) = glm::rotate((*model), 1.0f, m_transform.rotation);
	}
